...
| `ackMessage` | 在代理處理前傳送確認訊息（代理回覆後自動撤回） | `false` |
...
- **v3.7.0** — 入站防抖合併：接入 OpenClaw `messages.inbound.debounceMs`，合併同一使用者的連續快速訊息；確認訊息功能（`ackMessage` 配置）：代理處理前傳送「收到，正在處理...」，回覆後自動撤回，語言自動偵測
