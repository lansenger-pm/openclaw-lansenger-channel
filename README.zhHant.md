...
- **DynamicMsg appCard**——狀態更新格式：`appCardUpdateMsg` + `isLastUpdate` + `headStatusInfo`，用於審批狀態變更
...
| 變數 | 說明 | 範例 |
|------|------|------|
| `LANSENGER_APP_ID` | 個人機器人 App ID | `2285568-10117376` |
...
審批流程使用 **appCard** 傳送初始卡片，使用 **DynamicMsg appCard** 更新狀態：

- **初始傳送**：`msgType="appCard"`，使用 `isDynamic=true` + `headStatusInfo`（顯示待審批狀態，琥珀色）
...
| `appCard` | ✗ | ✓（`isDynamic=true`） | ✓ | 審批卡片（初始傳送） |
