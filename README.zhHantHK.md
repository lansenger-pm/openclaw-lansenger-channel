...
| `lansenger_send_approve_card` | 發送審批卡片，按鈕權限僅審批人可操作 |
...
> **API 網關地址必填**：必須透過 `openclaw.json` 配置或環境變數提供企業網關地址（如 `https://your-api-gateway.example.com`）— 見[設定說明](#設定說明)。
...
| `LANSENGER_API_GATEWAY_URL` | 藍信 API 網關 URL（必填） | — |

配置也可透過 `openclaw.json` 提供（見下方設定說明）。配置值優先；環境變數僅在配置未設定時作為回退。`apiGatewayUrl` 為必填項，無預設值。
...
| `groupAllowFrom` | 羣聊中允許觸發的用户 ID（非羣 ID）。藍信用户 ID 和羣 ID 格式相同，請從 API 或日誌確認。 | `[]` |
...
3. `account.homeChannel` — 機械人主人（從第一條私聊自動偵測）
...
或依賴自動偵測的機械人主人（`homeChannel`），首次私聊後自動設定。
...
- **網關 URL 為必填項** — 您必須提供企業網關地址（如 `https://your-api-gateway.example.com`），無預設值。
...
- **代理工具** — 代理工具（`lansenger_send_*`）已內建於此頻道插件——頻道設定且執行時始終可用。CLI 命令（`lansenger message send-*`）需要 `pipx install lansenger-cli`，是可選替代方案。
