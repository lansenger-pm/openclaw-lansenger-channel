...
function getSection(cfg: OpenClawConfig): Record<string, any> | undefined {
  return (cfg.channels as Record<string, any>)?.["lansenger"];
}
...
export const lansengerSetupWizard: any = {
  ...
  textInputs: [
    {
      ...
      required: false,
      initialValue: ({ cfg }: any) => {
        const section = getSection(cfg);
        return section?.apiGatewayUrl || "https://open.e.lanxin.cn/open/apigw";
      },
  ...
  finalize: async ({ cfg }: any) => {
    const channels = { ...((cfg.channels as Record<string, any>) ?? {}) };
    const current = channels.lansenger ?? {};
    channels.lansenger = { ...current, enabled: true, dmSecurity: current.dmSecurity ?? "paired" };
    return { cfg: { ...cfg, channels } };
  },
  ...
  dmPolicy: createTopLevelChannelDmPolicy({
    ...
    promptAllowFrom: async ({ cfg, prompter, accountId }: any) => {
      const section = getSection(cfg) ?? {};
      const existing: string[] = section.allowFrom ?? [];
  ...
  disable: (cfg: any) => setSetupChannelEnabled(cfg, "lansenger", false),
};